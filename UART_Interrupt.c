...
extern unsigned int Parat_Update_Flag;//图像解析参数数据
...
interrupt void KeyStone_UART_Niu_Rx_ISR()
{
	Uint32 rx_cnt = 0;
	Uint32 echo_cnt=0,i;
    ...
    if(CIC_Regs->RAW_STATUS_REG[CSL_INTC0_URXEVT>>5] & (1<<(CSL_INTC0_URXEVT%32)))
    {
		...
		//echo back the received characters
		if ((UART_Rx_Buf[0] == Img_Rdy_Head1) && (UART_Rx_Buf[1] == Img_Rdy_Head2))
			Rec_Img_num++;
		else if ((UART_Rx_Buf[0] == Parat_Update_Head1) && (UART_Rx_Buf[1] == Parat_Update_Head2))
			Parat_Update_Flag = 1;
		...
		GPIOPinWrite(SOC_GPIO_0_REGS, GPIO14_PCIESSMODE0, GPIO_PIN_HIGH);

		KeyStone_UART_write(UART_Echo_Buf, echo_cnt, 0);
//		KeyStone_UART_TX_wait(0);
		Delay(10000);
		GPIOPinWrite(SOC_GPIO_0_REGS, GPIO14_PCIESSMODE0, GPIO_PIN_LOW);
